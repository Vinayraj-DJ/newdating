// src/pages/WeeklyRewards/WeeklyRewards.jsx
import React, { useState, useEffect, useMemo } from "react";
import styles from "./WeeklyRewards.module.css";
...
const WeeklyRewards = () => {
  ...
  const fetchPendingRewards = async () => {
    ...
    setError("");
    try {
      const response = await getPendingWeeklyRewards();
  ...
  const handleTriggerWeeklyRewards = async () => {
    ...
    try {
      ...
      await fetchData();
      showCustomToast("Weekly rewards triggered successfully!");
    } catch (err) {
      setError(err.message || "Failed to trigger weekly rewards");
      console.error("Error triggering weekly rewards:", err);
    } finally {
  ...
  }, [currentPendingData, actionLoading, startIdxPending, handleApproveReward, handleRejectReward]);
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h2 className={styles.title}>Weekly Rewards Management</h2>
        <Button
          variant="primary"
          onClick={handleTriggerWeeklyRewards}
          disabled={loading.trigger}
          className={styles.triggerBtn}
        >
          {loading.trigger ? "Processing..." : "Trigger Weekly Rewards"}
        </Button>
      </div>
          ...
          {activeTab === "pending" && (
            <div className={styles.section}>
              {loading.pending ? (
                ...
                <div className={styles.emptyState}>No pending weekly rewards found.</div>
              ) : (
                <>
                  <DynamicTable
